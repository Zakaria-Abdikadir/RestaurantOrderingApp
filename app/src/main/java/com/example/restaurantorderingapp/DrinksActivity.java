...
public class DrinksActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void setupDrinksGrid() {
        ...
        for (int i = 1; i <= 8; i++) {
            ...
            ImageLoader.loadDrinkImage(this, imageView, i);
            
            textView.setText("Drink " + i);
            priceView.setText("$" + String.format("%.2f", (5 + i * 1.5 + 0.99)));

            final int itemIndex = i;
            final String imageResource = "drink" + i;
            final double price = 5 + i * 1.5 + 0.99;

            cardView.setOnClickListener(v -> {
                CartItem item = new CartItem("Drink " + itemIndex, imageResource, price, 1, "drink");
                CartManager.getInstance().addItem(item);
                Toast.makeText(this, "Added to cart: Drink " + itemIndex, Toast.LENGTH_SHORT).show();
